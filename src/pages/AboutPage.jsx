...
const TAGLINE = 'Full-Stack Developer & Creative Engineer';
...
const LINKS = [
  { label: 'GitHub',   href: 'https://github.com/YOUR_USERNAME',              icon: GitHubIcon },
  { label: 'LinkedIn', href: 'https://linkedin.com/in/YOUR_USERNAME',          icon: LinkedInIcon },
...
const SKILLS = [
  {
    category: 'Frontend',
    items: ['React', 'Next.js', 'TypeScript', 'Three.js / R3F', 'GSAP', 'Tailwind CSS'],
  },
  {
    category: 'Backend',
...
const TIMELINE = [
  {
    ...
    place: 'Celestica',
    description: 'Leading full-stack development on an internal video-generation platform — node-graph UI, real-time API orchestration, and design systems.',
  },
  {
    year: '2025',
    title: 'Software Developer',
    place: 'Arkhet AI Inc.',
    description: 'Shipped interactive React apps with complex state management, GSAP animations, and performance optimisations.',
  },
  {
    year: '2024',
    title: 'Frontend Developer',
    place: 'Magnify Access',
    description: 'Built a Figma-based frontend design system for a WCAG-compliant web application.',
  },
  ...
  return (
    <div className="w-full h-full flex flex-col bg-brand-brown-dark shadow-brand">
      ...
      <div className="px-10 py-6 shrink-0 bg-brand-gradient">
        ...
        <p className="text-sm text-brand-cream/70 mt-1">{TAGLINE}</p>
      </div>
            ...
            {TIMELINE.map(({ year, title, place, description }) => (
              <div key={`${year}-${title}`} className="pl-6 relative">
                ...
                <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5 mb-1">
                  <span className="text-xs font-mono text-brand-accent-brown">{year}</span>
                  <h3 className="text-sm font-semibold text-brand-cream">{title}</h3>
                  <span className="text-xs text-brand-tan">@ {place}</span>
                </div>
                <p className="text-xs text-brand-tan leading-relaxed">{description}</p>
              </div>
            ))}
          </div>
        </section>
